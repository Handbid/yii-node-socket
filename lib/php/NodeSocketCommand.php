 ...
 * yii node-socket/stop
   Stop socket server

 * yii node-socket/restart
   Restart socket server

 * yii node-socket/getPid
   Display socket pid
EOD;
    }
    ...
    protected function compileServer()
    {
        ...
        ob_start();
        include str_replace('/php', '/js/server/server.config.js.php', __DIR__);
        $js = ob_get_clean();
        return file_put_contents(str_replace('/php', '/js/server/server.config.js', __DIR__), $js);
    }
    ...
    protected function compileClient()
    {
        ...
        $nodeSocket = $this->getComponent();
        ob_start();
        include str_replace('/php', '/js/client/client.template.js', __DIR__);
        $js = ob_get_clean();
        return file_put_contents(str_replace('/php', '/js/client/client.js', __DIR__), $js);
    }
