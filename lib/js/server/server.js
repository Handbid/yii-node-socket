...
console.log(ts(), '[DEBUG] Server.js loaded');
...
// Socket.IO setup
// DIAGNOSTIC: Increased ping timeouts to test if Go consumer PING/PONG issue is root cause
// Default: pingInterval=25000, pingTimeout=20000
// See: /Users/jhporte/.claude/plans/glittery-tumbling-fairy.md
const io = new Server(server, {
	...
	allowEIO3: true,
	pingInterval: 60000,  // 60s (default 25s) — give PHP consumers more idle time
	pingTimeout: 45000    // 45s (default 20s) — total tolerance: 105s before disconnect
});
...
io.of('/client').use((socket, next) => {
	...
	// Debug: Log all events from browser clients
	socket.onAny((eventName, ...args) => {
		console.log(ts(), '[CLIENT NS] Received event:', eventName, 'from:', socket.id);
	});
});

// /server namespace
console.log(ts(), '[DEBUG] Setting up /server namespace');
io.of('/server').use((socket, next) => {
	console.log(ts(), '[SERVER NS] Middleware hit! Address:', socket.handshake.address);
	...
	console.log(ts(), '[SERVER NS] Allowed servers:', serverConfiguration.allowedServers, 'Is allowed:', allowed);

	if (!allowed) {
		console.log(ts(), '[SERVER NS] ERROR: Invalid server:', address);
		return next(new Error('INVALID SERVER: ' + address));
	}
	socket.handshake.sid = address;
	console.log(ts(), '[SERVER NS] Middleware passed, calling next()');
	next();
}).on('connection', socket => {
	console.log(ts(), '[SERVER NS] New connection from:', socket.handshake.address);

	// Track health state and catch all events from PHP/Go
	socket.onAny((eventName, ...args) => {
		console.log(ts(), '[SERVER NS] Received event:', eventName, JSON.stringify(args).substring(0, 500));
		health.lastEventAt = Date.now();
		health.eventsProcessed++;
		if (eventName === 'heartbeat') {
			health.lastHeartbeatAt = Date.now();
			console.log(ts(), '[HEALTH] Heartbeat received');
		}
	});
...
// /healthz endpoint — monitored by Uptime Robot
...
server.listen(serverConfiguration.port, serverConfiguration.host, () => {
	console.log(ts(), `[SERVER] Listening on ${serverConfiguration.host}:${serverConfiguration.port}`);
});
