...
io.of('/server').use((socket, next) => {
	...
	if (!address) return next(new Error('NO ADDRESS TRANSMITTED'));

	const allowed = serverConfiguration.allowedServers.includes(address);
	if (!allowed) return next(new Error('INVALID SERVER: ' + address));
