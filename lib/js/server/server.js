...
if (relay.redisUrl) {
	...
	// pingInterval: Redis closes idle clients after `timeout` (300 s in QA/prod); only the
	// instance PHP dials sees traffic, the others' pub connections would drop every 5 minutes.
	const pub = createClient({ url: relay.redisUrl, pingInterval: 60000 });
...
// the side port and uWS answers /, /livez and /healthz on the socket port (haproxy checks /livez).
let uws = null;
let uwsApp = null;
if (relay.engine === 'uws') {
	...
	const uwsLivez = (res) => {
		res.writeStatus('200 OK').writeHeader('content-type', 'application/json').end(JSON.stringify(livez()));
	};
	uwsApp.get('/healthz', uwsHealthz).get('/livez', uwsLivez).get('/', uwsLivez);
	io.attachApp(uwsApp, {
...
io.of('/server').use((socket, next) => {
	...
	// Track health state from PHP/Go events. PHP and the Go consumer dial one instance, so
	// with N processes only that one sees the heartbeat — share it with the peers, otherwise
	// the others report /healthz degraded after the startup grace.
	socket.onAny((eventName, ...args) => {
		...
		if (args[0] && args[0].data && args[0].data.type === 'heartbeat') {
			...
			if (relay.redisUrl) io.of('/client').serverSideEmit(HEARTBEAT_SHARE, now);
		}
	});
...
// Heartbeat seen by a peer instance (see the /server namespace above)
const HEARTBEAT_SHARE = 'relay:heartbeat';
if (relay.redisUrl) {
	io.of('/client').on(HEARTBEAT_SHARE, (at) => {
		const t = Number(at) || Date.now();
		if (!health.lastHeartbeatAt || t > health.lastHeartbeatAt) health.lastHeartbeatAt = t;
		if (!health.lastEventAt || t > health.lastEventAt) health.lastEventAt = t;
	});
}
...
// /livez — liveness only, always 200 while the process serves: the haproxy backend check.
// /healthz turns 503 when the PHP heartbeat is stale, which must page (Uptime Robot), not
// pull every relay instance out of the backend at once.
const livez = () => ({ status: 'alive', engine: relay.engine, instance: relay.instance, port: relay.port });
app.get('/livez', (req, res) => res.status(200).json(livez()));
