...
io.of('/client').use((socket, next) => {
	...
	console.log('[CLIENT NS] New browser connection:', socket.id, 'from:', socket.handshake.address);
	...
	socket.on('disconnect', (reason) => {
		console.log('[CLIENT NS] Disconnected:', socket.id, 'reason:', reason);
	});

	// Debug: Log all events from browser clients
	socket.onAny((eventName, ...args) => {
		console.log('[CLIENT NS] Received event:', eventName, 'from:', socket.id);
	});
});
